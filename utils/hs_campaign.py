...
def get_campaign_ids(json_file, surface):
    ...
    try:
        ...
        style_message(f" {' '*7} * Campaign ID: {campaign_id}", 'cyan')
        style_message(f" {' '*7} * Variant ID: {variant_id}", 'cyan')
        ...
        style_message(f" {' '*7} * Error Code: {errorCode}", 'red')
        style_message(f" {' '*7} * Error Message: {errorMessage}", 'red')
